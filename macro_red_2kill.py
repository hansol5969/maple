...
def collection_route():
    """v2 회수 루트 — 80초마다.
    [1] 231-245 정렬 → [2-4] up_teleport ×1 (→ Y=140)
    [5] Y=139-141 시 attack → [6] z bounce 1회왕복 → [7] drop×2 → 1층
    ...
    # [2-4] 위 텔포 1회 → Y=140 안착
    print('[collect-v2] [2-4] up_teleport ×1')
    up_teleport()
    rsleep(0.55, 0.1)
    if STOP:
        return

    # [5] Y=139-141 안착 시에만 attack
    pos = find_char_minimap_pos(grab())
    if pos and 139 <= pos[1] <= 141:
        print(f'[collect-v2] [5] mm={pos} → attack')
        tap(ATTACK_KEY, ATTACK_DOWN_SECONDS)
    else:
        print(f'[collect-v2] [5] mm={pos} Y가 139-141 아님 → attack 생략')

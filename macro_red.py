...
SAFE_ZONE_TARGET_Y       = 83   # 안전지대 도착 판정 mm_y (이 값 이하면 도착) — 사용자 측정
# 안전지대 전체 X 범위 (사용자 측정 X=174-202, Y=83)
# 그 중 포탈 영역(X=180-192)은 안전지대에서 제외 — 위 텔포 시전 금지
# 위 텔포 시전 가능: 우측 영역 X=193-202 (포탈 회피)
SAFE_ZONE_X_FULL         = (174, 202)  # 참고용 — 안전지대 전체 (사용자 측정)
PORTAL_DANGER_X          = (180, 192)  # 포탈 영역 — 위 텔포 시전 금지 (사용자 측정)
...
# UI 좌표 캘리브레이션 — 새 환경 ui_probe.py 직접 측정 (게임창 평행이동만 보정)
COORDS_CAL_GAME_LEFT = -11     # 측정 시점 게임창 left
COORDS_CAL_GAME_TOP  = 0       # 측정 시점 게임창 top
# 게임 월드 픽셀 거리 캘리브레이션 — 옛 환경 기준 (AOE/사거리 등 자동 비율 보정용)
COORDS_CAL_GAME_W    = 3126    # 옛 게임창 width (픽셀 거리 비율 보정)
COORDS_CAL_GAME_H    = 1758    # 옛 게임창 height (픽셀 거리 비율 보정)
...
ETC_SELL_BUTTON_ABS  = (1348, 338)   # "팔기" 버튼 — 클릭 후 ENTER 반복 (캘리브레이션 좌표, 화면 (1369,360))
...
def _abs(x: int, y: int) -> tuple:
    """캘리브레이션 시점 절대 좌표 → 현재 게임창 위치 평행이동.
    UI는 게임 UI 비율(게임창과 다름)로 ui_probe.py 직접 측정했기에 스케일링 안 함."""
...
def _game_rel(x: int, y: int) -> tuple:
    """캘리브레이션 절대 좌표 → 현재 game grab 영역 내 상대 좌표 (평행이동만)."""
...
PATROL_LEFT_BOUND  = 74   # 좌측 끝 (사용자 측정)
PATROL_RIGHT_BOUND = 191  # 우측 끝 (사용자 측정, 안전지대 잠수 회피용)
...
# 각 층 mm_y 좌표 (옛 값 × 0.625)
FLOOR1_Y       = 119  # 2층 우측 끝에서 낙하한 1층 (가장 아래) — 사용자 측정
FLOOR2_Y       = 100  # 2층 (사냥라인) — 사용자 측정, minimap_config_red.floor1_y_baseline 와 동일 의미
FLOOR2_STAIR_Y = 91   # 2층계단 (2층과 3층 사이 작은 platform, 사용자 측정)
FLOOR3_Y       = 82   # 3층
FLOOR3_TOP_Y   = 72   # 3층에서 위+텔포 후 아래점프 도착 platform
FLOOR2_RIGHT_TOP_Y = 89   # 2층 우측에서 위+텔포 도착 platform (jump+drop 후 안착, z bounce 위치, 사용자 측정)
COLLECT_TOP_PLATFORM_Y = 78   # 회수 [2-4] hold_up_teleport 최종 도달 platform
FLOOR_Y_TOL    = 4    # Y 매칭 허용 오차

# 회수 X 좌표 (옛 값 × 0.625)
ROUTE_F1_UPTP_X      = (59, 67)    # 1층계단 회수 + 위+텔포 시도 X 범위 (사용자 측정)
ROUTE_F2_STAIR_X     = (92, 99)    # 2층계단 점프 + z hold 왕복 범위 (사용자 측정)
ROUTE_F3_X           = (108, 117)  # 3층 z hold 왕복 범위 (사용자 측정, y=72)
ROUTE_F2_RIGHT_X     = (142, 151)  # 2층 우측 위+텔포 + z hold 왕복 범위 (HUNT_X와 동일)
...
HUNT_X_MIN = 142  # collection 정렬용 / z hold platform 좌측 (사용자 측정)
HUNT_X_MAX = 151  # z hold platform 우측 마진 (사용자 측정)
COLLECT_F1_PATROL_X = (119, 223)  # 1층 텔포공격 왕복 범위 (사용자 측정)
COLLECT_F1_UPTP_X   = (59, 67)    # 1층 위+텔포 시도 X — 계단 위치 (사용자 측정)
COLLECT_F1_CLEAR_X  = (59, 78)    # 1층 up_tp 직전 몹 정리 X 범위 (UPTP 좌측 일치, 우측 약간 더)
COLLECT_F1_LEFT_STOP_X = 150      # 1층 도착 후 먼저 찍는 X (좌측 회수, 왕복 시 동선 단축)

# 3층 추가 회수 루트 (옛 값 × 0.625)
ROUTE_3F_ENTRY_X        = 114          # 3층 진입 위 텔포 시작 X (옛 182)
ROUTE_3F_TOP_Y          = 64           # hold_up_teleport×2 후 도착 mm_y (옛 102)
ROUTE_3F_PLATFORM_X     = (108, 117)   # 3층 z bounce 범위 (사용자 측정)
ROUTE_3F_PLATFORM_Y     = 72           # 사용자 측정
ROUTE_3F_DROP_TARGET_X  = 101          # 3층→2층 계단 가는 X (옛 161)
ROUTE_3F_DROP_TARGET_Y  = 82           # 옛 131
ROUTE_F2_STAIR_X        = (92, 99)     # 2층 소규모 계단 z bounce (사용자 측정)
ROUTE_F2_STAIR_Y        = 91           # 사용자 측정

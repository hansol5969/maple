...
# 왼쪽에서 접근 시 사용하는 엄격한 left bound — X 정렬이 덜 된 채 zone 좌측 진입하면
# 실제 에스컬레이터 진입점에 못 닿아서 위+텔포가 헛타는 문제 방지.
# 오른쪽에서 접근하는 경우엔 ESCALATOR_X_MIN(410) 그대로 사용.
ESCALATOR_X_MIN_FROM_LEFT = 425
...
def hold_all(direction: str):
    """이동키 + 텔포 + 공격 모두 hold (실제 사용자 플레이 패턴)."""
    hold(direction)
    tp_hold()
    attack_hold()
...
def goto_escalator() -> bool:
    ...
    last_attack = 0.0
    last_tp = 0.0
    align_min = None  # 첫 위치 잡힌 뒤 접근 방향에 따라 결정
    ...
    while not STOP and time.time() - start < GOTO_ESC_TIMEOUT \
        ...
        # 첫 위치에서 접근 방향 결정. 왼쪽에서 들어올 때만 align_min을 더 오른쪽으로 당김.
        if align_min is None:
            if mm_x < target_min:
                align_min = ESCALATOR_X_MIN_FROM_LEFT
                print(f'[goto-esc] 왼쪽 접근 (mm_x={mm_x}) → align_min={align_min}')
            else:
                align_min = target_min
                print(f'[goto-esc] 오른쪽/안 접근 (mm_x={mm_x}) → align_min={align_min}')

        # 정렬됨 → 위+텔포
        if align_min <= mm_x <= target_max:
            tries += 1
        ...
        # 정렬 이동 — 접근 방향에 맞춘 zone 중심으로 향하게
        align_center = (align_min + target_max) // 2
        dx = align_center - mm_x
...
# 사냥 라인 양 끝 mm_x 범위 (mm_probe로 측정) — 끝 도달 즉시 방향 반전
PATROL_LEFT_BOUND  = 36   # 이 값 이하면 좌측 끝 → right로 반전
PATROL_RIGHT_BOUND = 485  # 이 값 이상이면 우측 끝 → left로 반전
    ...
    while not STOP:
        ...
        if mm_y is not None and floor1_y is not None and mm_y < floor1_y - FLOOR2_DELTA_Y:
            if on_floor2_since is None:
                ...
                print(f'[*] 2층 감지 (mm_y={mm_y}) → 즉시 키 릴리스 + 아래 점프')
                release_all()  # hold 키들 먼저 떼기 — 이게 우선
                rsleep(0.05, 0.2)  # 키 떼는 시간 확보
                drop_down()
                last_drop_down = now
                on_floor2_since = now
                last_mm_move_ts = now
